...
export const sendMessage = async (req, res) => {
    ...
    try {
      ...
      // Popola il campo sender (nome dell'utente) senza execPopulate
      await newMessage.populate('sender', 'name');
  
      // Emittiamo il messaggio tramite socket, includendo il nome del mittente
      emitGlobalEvent(
        'message',
        {
          content: newMessage.content,
          sender: newMessage.sender.name, // Nome del mittente incluso
          tripId,
      ...
      // Includiamo il nome del mittente nella risposta
      res.status(201).json({
        _id: newMessage._id,
        content: newMessage.content,
        sender: newMessage.sender.name, // Nome del mittente
        tripId: newMessage.trip,
        timestamp: newMessage.timestamp,
      });
    } catch (error) {
  ...
  export const getMessages = async (req, res) => {
    ...
    try {
      ...
      // Popoliamo il campo sender per ottenere il nome del mittente
...
export const deleteMessage = async (req, res) => {
    ...
    try {
      ...
      if (!isSender && !isAdmin) {
        return res.status(403).json({ error: 'Non sei autorizzato a eliminare questo messaggio' });
      }
  
      // Usa findByIdAndDelete per eliminare il messaggio
  ...
  export const editMessage = async (req, res) => {
    ...
    try {
      ...
      if (!message.sender.equals(userId)) {
        return res.status(403).json({ error: 'Non sei autorizzato a modificare questo messaggio' });
      }
